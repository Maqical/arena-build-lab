...
type Worker = "riot" | "pros" | "youtube" | "data";
type PatchStatus = { localPatch: string; livePatch: string; localDisplayPatch: string; liveDisplayPatch: string; stale: boolean; checkedAt: string; error?: string };
...
export function Settings() {
  ...
  const [twitchLogins, setTwitchLogins] = useState("");
  const [opacity, setOpacity] = useState(100);
  const [scale, setScale] = useState(1);
  const [notifyProMatches, setNotifyProMatches] = useState(true);
  const [notifyPatch, setNotifyPatch] = useState(true);
  ...
  useEffect(() => {
    void window.arenaDesktop?.getSettings().then((settings) => {
      ...
      setTwitchLogins(settings.twitchLogins);
      setOpacity(settings.opacity * 100);
      setScale(settings.scale);
      setNotifyProMatches(settings.notifyProMatches);
      setNotifyPatch(settings.notifyPatch);
      setNotifyRecords(settings.notifyRecords);
    });
  ...
  const save = async () => {
    setStatus("Saving…");
    const result = await window.arenaDesktop?.saveSettings({ riotId, riotApiKey, openAiApiKey, twitchClientId, twitchClientSecret, twitchLogins, opacity: opacity / 100, scale, notifyProMatches, notifyPatch, notifyRecords });
  ...
  const runWorker = async (worker: Worker) => {
    const label = worker === "riot" ? "Riot match" : worker === "pros" ? "Tracked player" : worker === "youtube" ? "Video catalog" : "Data Dragon";
  ...
  return <main className="settings-shell">
    ...
    <section className="settings-card"><h2>Notifications</h2><label className="settings-check"><input type="checkbox" checked={notifyProMatches} onChange={(event) => setNotifyProMatches(event.target.checked)} /> Tracked-player Arena activity</label><label className="settings-check"><input type="checkbox" checked={notifyPatch} onChange={(event) => setNotifyPatch(event.target.checked)} /> New patch data sync</label><label className="settings-check"><input type="checkbox" checked={notifyRecords} onChange={(event) => setNotifyRecords(event.target.checked)} /> Personal stat records</label><p>Preferences are applied when you save settings.</p></section>
    {patchStatus?.stale && <button className="settings-patch-warning" type="button" onClick={() => void runWorker("data")} disabled={!desktop || workerBusy !== null}><strong>New League patch {patchStatus.liveDisplayPatch || patchStatus.livePatch} detected</strong><span>Your local data is patch {patchStatus.localDisplayPatch || "not synced"} (data build {patchStatus.localPatch || "unknown"}). Click to sync.</span></button>}
    ...
    <section className="settings-card"><h2>Tracked players</h2><p>Resolve the public seed registry, refresh recent Arena matches, and queue local follow notifications.</p><div className="settings-actions"><button type="button" onClick={() => void runWorker("pros")} disabled={!desktop || workerBusy !== null}>{workerBusy === "pros" ? "Syncing..." : "Sync Tracked Players"}</button><a href="/pros">Open Pro Watch</a></div><label>Twitch Client ID<input type="password" value={twitchClientId} onChange={(event)=>setTwitchClientId(event.target.value)} autoComplete="off" /></label><label>Twitch Client Secret<input type="password" value={twitchClientSecret} onChange={(event)=>setTwitchClientSecret(event.target.value)} autoComplete="off" /></label><label>Tracked Twitch logins<input type="text" value={twitchLogins} onChange={(event)=>setTwitchLogins(event.target.value)} placeholder="login_one, login_two" autoComplete="off" /></label></section>
    <section className="settings-card"><div className="settings-about-heading"><h2>About</h2><span className="settings-version">v1.1.1</span></div><p>Arena Build Lab by Frbz.gg runs locally and reads the supported local client surfaces without game injection.</p><p>Published by Froobs, LLC. Updates are installed with a newer setup package.</p></section>
  </main>;
}
