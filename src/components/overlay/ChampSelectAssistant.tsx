...
function normalized(value: string): string { return value.toLowerCase().replaceAll(/[^a-z0-9]+/g, ""); }
function display(value: number, digits = 0): string { return value.toLocaleString(undefined, { maximumFractionDigits: digits, minimumFractionDigits: digits }); }
...
export function ChampSelectAssistant({ champions, snapshot: externalSnapshot, compact = false }: { champions: Champion[]; snapshot?: GameStateSnapshot | null; compact?: boolean }) {
  ...
  if (!championReference || !recommendation) return <section ref={rootRef} className={`champ-select-assistant ${compact ? "compact" : ""}`}><div className="champ-assistant-empty"><span>{snapshot?.phase === "champ_select" ? "Waiting for your hover..." : "Champion Select assistant armed"}</span><p>Hover or lock a champion in Arena to load local builds automatically.</p>{!compact && <label>Preview<select aria-label="Preview champion" value={previewChampion} onChange={(event) => setPreviewChampion(event.target.value)}><option value="">Choose a champion...</option>{champions.map((champion) => <option value={champion.id} key={champion.id}>{champion.name}</option>)}</select></label>}{chosenPreview && <small>Loading {chosenPreview.name}...</small>}{error && <small>{error}</small>}</div></section>;
  ...
  return <section ref={rootRef} className={`champ-select-assistant ${compact ? "compact" : ""}`}>
    ...
    <div className="champ-assistant-section"><div className="overlay-section-title"><span>Difficult field matchups</span><b>Placement edge</b></div><div className="champ-duos">{recommendation.difficultMatchups.slice(0, compact ? 2 : 3).map((matchup) => <article key={matchup.championId}><Image src={matchup.iconUrl} width={compact ? 28 : 38} height={compact ? 28 : 38} alt="" unoptimized /><div><strong>{matchup.name}</strong><span>{(matchup.aheadRate * 100).toFixed(1)}% ahead · {matchup.games} games</span></div></article>)}</div></div>
    ...
    {!compact && <p className="champ-assistant-note">{recommendation.note}</p>}{error && <p className="overlay-error">{error}</p>}
  </section>;
}
