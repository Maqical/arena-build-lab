...
export function ChampSelectAssistant({ champions, snapshot: externalSnapshot, compact = false }: { champions: Champion[]; snapshot?: GameStateSnapshot | null; compact?: boolean }) {
  ...
  if (!championReference || !recommendation) {
    return <section className={`champ-select-assistant ${compact ? "compact" : ""}`}>
  ...
  return <section className={`champ-select-assistant ${compact ? "compact" : ""}`}>
    ...
    <div className="champ-assistant-section"><div className="overlay-section-title"><span>Duo role-fit</span><b>Local meta</b></div><div className="champ-duos">{recommendation.duoRecommendations.slice(0, compact ? 3 : 4).map((duo) => <article key={duo.championKey}><Image src={duo.iconUrl} width={compact ? 28 : 38} height={compact ? 28 : 38} alt="" unoptimized /><div><strong>{duo.name}</strong><span>{duo.fitTags.join("/")} · {duo.winRate?.toFixed(1)}% · {duo.tier}</span></div></article>)}</div></div>
