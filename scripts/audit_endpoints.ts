...
const checks: Array<{ path: string; method?: string; body?: unknown }> = [
  ...
  { path: "/api/augment-builds?championId=14&augmentIds=1" },
  { path: "/api/insights?kind=tier&region=na" }, { path: "/api/pros?region=global" },
  { path: "/api/notifications" }, { path: "/api/streams" }, { path: "/api/meta-report?region=na" },
];
